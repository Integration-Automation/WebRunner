...
除了核心 WebDriver / 動作執行器外,WebRunner 另外提供大量專用模組,
每一個都在 `je_web_runner/utils/<area>/` 底下獨立成包,並附有完整單元
測試。依能力分類:

### Web 平台 API

- **`webtransport_assert`** — HTTP/3 WebTransport datagram + stream 斷言
- **`indexed_db_explorer`** — IndexedDB 快照與物件 / 索引斷言
- **`file_system_access`** — 模擬 `showOpenFilePicker` / `showSaveFilePicker` 並記錄寫入
- **`notifications_audit`** — 追蹤 `Notification.requestPermission` 呼叫時機與政策
- **`sse_assert`** — Server-Sent Events stream 錄製 + count / data / id 斷言
- **`websocket_assert`** — WebSocket frame 錄製 + count / payload / pubsub 斷言
- **`webrtc_assert`** — PeerConnection 狀態 / ICE / track / RTP 斷言
- **`view_transitions`** — View Transitions API duration / CLS / group 斷言

### 安全 / Headers

- **`mixed_content_audit`** — HTTPS 頁面內 HTTP 資源偵測
- **`clickjacking_audit`** — X-Frame-Options + frame-ancestors + iframe 探測
- **`open_redirect_detector`** — 八種 payload 偵測 ?redirect= 類弱點
- **`sri_verify`** — Subresource Integrity hash 存在性 + 正確性驗證
- **`coop_coep_audit`** — crossOriginIsolated COOP / COEP + 子資源 CORP/CORS 檢查
- **`token_leak_detector`** — 掃描 HAR / log / response 中的 JWT / AWS / GitHub token 等
- **`consent_audit`** — GDPR / CCPA cookie 分類 + 同意前洩漏 / 拒絕後重新植入偵測
- **`pii_in_screenshot`** — Screenshot OCR + 個資 (信用卡 / 身分證 / SSN) 掃描

### 效能預算

- **`inp_tracker`** — Interaction to Next Paint 測量 + p98 + 預算
- **`hydration_check`** — SSR hydration mismatch (DOM diff + console marker)
- **`bundle_budget`** — 每種資源 (script/css/image/font/media) 大小預算
- **`third_party_budget`** — 第三方廠商請求數 / 大小 / 阻塞時間預算
- **`long_animation_frame`** — Long Animation Frame API + 每 script 歸因
- **`console_error_budget`** — JS console / unhandled rejection 預算

### 後端整合

- **`grpc_tester`** — gRPC stub 呼叫錄製 + gRPC-Web framing / trailer
- **`webhook_receiver`** — 臨時 HTTP server 接收 app 對外 webhook + 斷言
- **`idempotency_check`** — 同一請求送兩次,比對 status / body / state / 副作用
- **`pagination_audit`** — 遍歷所有頁,偵測重複 / 漏抓 / cursor loop / 排序錯誤
- **`backend_log_correlator`** — W3C trace_id → Loki / Elasticsearch / 檔案 log 拉取
- **`email_render`** — MailHog / Mailpit / `.eml` 攔截 → 多 viewport 截圖

### AI / 工作流

- **`failure_narrator`** — LLM 將 failure bundle 轉為自然語言失敗摘要
- **`repro_minimizer`** — Delta-debugging (ddmin) 縮減失敗 action list
- **`locator_hardener`** — Fragility 評分 + LLM 建議更穩定的 selector
- **`test_categorizer`** — 自動標記 smoke / regression / perf / a11y / security
- **`exploratory_ai`** — Agent-style 探索測試員 + 確定性 RandomPlanner
- **`story_to_actions`** — 使用者故事 / Figma frame → 驗證後的 WR action JSON
- **`session_to_test`** — rrweb / 通用 event stream → WR action JSON
- **`multimodal_qa`** — Screenshot + 問題送 vision LLM 取得 pass/fail 判定
- **`prompt_drift_monitor`** — 追蹤 app 內 LLM 功能的輸出飄移
- **`test_dedup_ai`** — 結構性 + 語意 embedding 去重 action JSON

### 無障礙 / 國際化 / 視覺

- **`ocr_assert`** — Canvas / WebGL / 圖片內 OCR 文字斷言
- **`screen_reader_runner`** — 走訪 a11y tree 模擬 NVDA / VoiceOver 朗讀順序
- **`pseudo_localization`** — 字串偽本地化 + 偵測硬編碼字串
- **`forced_colors_mode`** — Dark / reduced-motion / forced-colors / 高對比矩陣

### 治理與報告

- **`pr_risk_score`** — 融合 flake / impact / locator / coverage 訊號為 0-100 分
- **`flag_matrix`** — Feature flag 組合矩陣 + 失敗最小子集分析
- **`chaos_hooks`** — 種子化 chaos 注入 (離線 / throttle / 中途 reload)
- **`db_snapshot`** — 每個 test 自動 DB savepoint / rollback
- **`time_freezer`** — CDP 注入腳本凍結 `Date` / `performance.now`
- **`persona_runner`** — 同一份 suite × N 種角色矩陣
- **`git_bisect_flake`** — Ledger / probe 兩種模式 bisect 出回歸 commit
- **`test_cost_estimator`** — 雲端分鐘 × 費率 → USD + CO₂ 估算
- **`slack_digest`** — Slack Block Kit / Teams Card / 純文字測試週報
- **`quarantine_age_report`** — Quarantine 條目 + age + 升級 tier
- **`test_debt_dashboard`** — skip / xfail / TODO 盤點 + age + 負責人
- **`sla_tracker`** — 「Y% suite 在 X 分鐘內跑完」週 / 日趨勢
- **`bug_repro_stability`** — N 次重複 + 分類 deterministic / flaky / 無法重現
- **`test_owners_map`** — CODEOWNERS 解析 + 覆寫層 + 無人領養稽核

完整模組樹見 [`CLAUDE.md`](../CLAUDE.md);完整命令清單見英文版
[README.md](../README.md) 與 `docs/source/Eng/doc/specialized_modules/`。
