...
The tree below lists the main packages. Every `utils/` subpackage with a one-line summary, by layer
(core engine, `je_web_runner.api` facade themes, other): `docs/reference/utils_index.md`, generated by
`python scripts/gen_utils_index.py`; `test/unit_test/test_utils_index.py` fails when it is stale.
