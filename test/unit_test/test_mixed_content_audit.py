...
class TestScanHar(unittest.TestCase):
    ...
    def test_active_finding(self):
        findings = scan_har(
            _har(_har_entry("http://x.com/bad.js", "script")),
            page_url="https://x.com",
        )
    ...
    def test_passive_finding(self):
        findings = scan_har(
            _har(_har_entry("http://x.com/img.png", "image")),
            page_url="https://x.com",
        )
    ...
    def test_upgrade_for_hsts_domain(self):
        findings = scan_har(
            _har(_har_entry("http://fonts.googleapis.com/css", "stylesheet")),
            page_url="https://x.com",
        )
    ...
    def test_unknown_resource_type_active(self):
        findings = scan_har(
            _har(_har_entry("http://x.com/a", "weird")),
            page_url="https://x.com",
        )
        self.assertEqual(findings[0].severity, Severity.ACTIVE)
    ...
    def test_http_page_no_risk(self):
        findings = scan_har(
            _har(_har_entry("http://x.com/a.js")),
            page_url="http://x.com",
        )
        self.assertEqual(findings, [])
    ...
    def test_str_har(self):
        findings = scan_har(
            json.dumps(_har(_har_entry("http://x.com/a.js"))),
            page_url="https://x.com",
        )
        self.assertEqual(len(findings), 1)
    ...
    def test_page_url_inferred_from_har_pages(self):
        har = {
            "log": {
                ...
                "entries": [_har_entry("http://example.com/img.png", "image")],
            }
        }
    ...
    def test_no_page_url_assumes_https(self):
        ...
        findings = scan_har(_har(_har_entry("http://x.com/a.png", "image")))
        self.assertEqual(len(findings), 1)
...
class TestAssertions(unittest.TestCase):
    ...
    def test_assert_no_active_pass(self):
        assert_no_active([
            MixedFinding(url="http://x", resource_type="image",
    ...
    def test_assert_no_active_fail(self):
        with self.assertRaises(MixedContentAuditError):
            assert_no_active([
                MixedFinding(url="http://x", resource_type="script",
