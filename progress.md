...
- **#1** The UTF-8 log encoding in `je_web_runner/utils/logging/loggin_instance.py` (`WebRunnerLoggingHandler`, `encoding="utf-8"`) has no test and is not released: add a test that writes a non-ASCII line and reads it back as UTF-8, then release. Jeffrey_RPA loads this working tree through `sys.path` and waits for the release (workspace S-7, X-2).
